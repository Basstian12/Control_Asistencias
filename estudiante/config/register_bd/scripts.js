...
$(document).ready(function () {
    ...
    $("#btn_buscarAsistencia").click(function () {

        var materia = $("#select_materia").val();
        $.ajax({
            url: "../../config/register_bd/search_alumnasistencia.php",
            method: "POST",
            data: {
                materia: materia
            },
            success: function (result) {
                $("#resultado_asistencia").html(result);
            }
...
$(document).ready(function () {
    // Capturar evento de envío del formulario
    $('#form_asistencias').submit(function (event) {
        // Evitar el comportamiento por defecto del formulario
        event.preventDefault();
        ...
        $.ajax({
            type: 'POST',
            url: '../../config/register_bd/asistencia_materia.php',
            data: datos1,
            success: function (result3) {
                ...
                Swal.fire({
                    position: 'top-center',
                    icon: 'success',
                    title: 'Registraste una asistencia',
                    showConfirmButton: false,
                    timer: 2000
                });
            },
...
$(document).ready(function () {
    // Capturar evento de envío del formulario
    $('#form_inscripcion').submit(function (event) {
        // Evitar el comportamiento por defecto del formulario
        event.preventDefault();

        // Obtener los datos del formulario
        var formInscribirme = $(this).serialize();

        // Enviar los datos mediante AJAX
        $.ajax({
            type: 'POST',
            url: 'insertar.php',
            data: formInscribirme,
            success: function (inscribirme) {
                // Mostrar mensaje de éxito
                Swal.fire({
                    position: 'top-center',
                    icon: 'success',
            ...
            error: function () {
                // Mostrar mensaje de error
                 Swal.fire({
              icon: 'error',
              title: 'No se pudo inscribir, intentalo mas tarde',
              showConfirmButton: false,
              timer: 1500
            })
            }
